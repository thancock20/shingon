...
export function getOutputPath(type, entityName, moduleName) {
  ...
  } else if (type === 'method-stub') {
    return `./client/modules/${moduleName}/configs/method_stubs/${outputFileName}`;
  } else if (type === 'publication') {
    return `./server/publications/${outputFileName}`;
  } else if (type === 'storybook') {
    return `./client/modules/${moduleName}/components/.stories/${outputFileName}`;
  } else {
...
export function getTestOutputPath(type, entityName, moduleName) {
  ...
  if (type === 'model') {
    return `./tests/gagarin/${outputFileName}`;
  }
  return `./client/modules/${moduleName}/${type}s/tests/${outputFileName}`;
}

/**
 * Gets the path to the generic template inside Shingon given the type.
 * @param type {String} - type of the file being generated.
 * @return String - the absolute path to the generic template inside Shingon
...
export function getTemplatePath(type, options = {}) {
  ...
  if (options.testTemplate) {
    if (type === 'component') {
      ...
      relativePath = '../../templates/tests/gagarin/test.tt';
    }
  } else {
    if (type === 'collection') {
      if (options.model) {
        if (options.hasOwner) {
        relativePath = '../../templates/lib/collections/generic_model_owner.tt';
        } else {
          relativePath = '../../templates/lib/collections/generic_model.tt';
        }
      } else if (options.schema === 'collection2') {
      ...
      if (options.model) {
        relativePath = '../../templates/server/methods/generic_model.tt'
      } else {
        relativePath =  `../../templates/server/methods/generic.tt`;
      }
    } else if (type === 'method-stub') {
      relativePath = '../../templates/client/modules/core/configs/method_stubs/generic.tt'
    } else if (type === 'publication') {
      if (options.model) {
        if (options.hasOwner) {
          relativePath = '../../templates/server/publications/generic_model_owner.tt';
        } else {
          relativePath = '../../templates/server/publications/generic_model.tt';
        }
      } else {
 ...
 * @param config {Object} - global configuration for Shingon. This may contain
  ...
  if (type === 'component') {
    ...
    if (options.children) {
      ...
      options.children.forEach((child) => {
        const componentName      = _.upperFirst(_.camelCase(child));
        const componentFileName  = _.snakeCase(child);
        const importStatement    = `import ${ componentName } from './${ componentFileName }.jsx';`;
        const componentStatement = `<${componentName} />`;
        importsArray.push(importStatement);
        componentsArray.push(componentStatement);
      })
      const componentDelimeter = '\n' + ' '.repeat(options.useClass ? 9 : 4);
      variables.childrenImports = '\n' + importsArray.join('\n');
      variables.childrenComponents = componentDelimeter + componentsArray.join(componentDelimeter);
    }  else {
      variables.childrenImports = null;
      variables.childrenComponents = null;
    }
    ...
    if (options.schema === 'astronomy' || options.model) {
...
export function getTestTemplateVaraibles(type, moduleName, fileName) {
  if (type === 'action') {
    ...
    return {
      modelFileName: _.snakeCase(fileName)
    }
  }
}
...
export function updateIndexFile({indexFilePath, exportBeginning, insertImport, insertExport, commaDelimited, omitExport = false}) {
  ...
  if (indexContent.match(insertImport)) {
    logger.exists(`${insertImport} in ${indexFilePath}`);
    return;
  }
 ...
 * @param config {Object} - Shingon config
...
export function compileTemplate(content, variables, config) {
  ...
  return compiled.replace(/(\n|\r\n)( +)/g, function (match, lineBreak, defaultTab) {
    ...
    return lineBreak + tab.repeat(numTabs);
  });
}
